...
\begin{itemize}
    \item \textbf{Left (Example 1):} The model perfectly tracks the L-bracket profile, correctly predicting Lines and the closing Coincident constraint.
    \item \textbf{Center (Example 2):} The model handles the internal hole pattern correctly (Circles) but shows slight deviation on the flange height.
    \item \textbf{Right (Example 3):} This illustrates the main failure mode. The ground truth uses a specific Arc radius for the fillet, while the model predicts a slightly different curvature. However, critically, the model's uncertainty (not visualized here, but high in the logs) would allow a user to correct this.
\end{itemize}
...
\caption{Rollout gallery: predicted primitives (red solid) overlaid on ground-truth steps (blue dotted) for three representative brackets, read left-to-right. Large rendering makes alignment, fillets, and hole placement easy to inspect.}
\label{fig:progress}
\end{figure*}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/risk_curve.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/reliability_comparison.png}
